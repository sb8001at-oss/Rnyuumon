...
Quartoでは、PDFの文章の表示形式として、Document Classというものが設定できます。Document Classにはarticle、report、bookの3つがあります。
...
[`ggplot2`の章](./chapter26.html)に記載した通り、`ggplot2`で日本語を用いてPDFとして保存すると、日本語が文字化けします。日本語をきちんと表示させる方法はいろいろ提案されていますが（例えばQiitaの[この記事](https://qiita.com/yetanothersu/items/a5ad93574adb9208fb68)など）、Quartoではシンプルにグラフを画像（`png`や`jpg`）で出力してしまうのが比較的簡単です。
