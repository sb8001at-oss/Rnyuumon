...
指数分布に関しては[28章](chapter28.qmd#指数分布)でごく簡単に取り扱っています。指数分布は平均値1/λ、標準偏差も1/λとなる分布です。ただし、この指数分布では、**ハザードが時間に対して一定**である場合のみ生存時間をうまく表現することができます。
...
また、以下のshinyliveでも同様のアプリを実行できます。
[生存時間シミュレーター](https://surv-sim-shinylive.pages.dev/)
...
surv_ex1 |> knitr::kable(col.names=c("day", "リスク集団", "生存者数", "時点の生存割合", "カプランマイヤーの計算式", "生存時間"), align = "rrrrr")
```
...
surv_ex2 |> knitr::kable(col.names=c("day", "リスク集団", "生存者数", "時点の生存割合", "カプランマイヤーの計算式", "生存時間"), align = "rrrr")
```
...
```{r, filename="後期の生存時間に差がある場合", echo=FALSE}
set.seed(0)
n <- 125
...
sv_temp <- 
  data.frame(
    sv = c(
      rweibull(n, shape = 0.8, scale = 115) |> round(),
      rweibull(n, shape = 1.2, scale = 95) |> round()
    ),
    sex =rep(
      c("male", "female"),
      c(n, n)
    ),
    status = rep(c(0, 1), c(30, 70)) |> sample(n * 2, replace = TRUE)
  )
```


```{r, filename="後期の生存時間に差がある場合"}
...
```{r, filename="初期の生存時間に差がある場合", echo=FALSE}
set.seed(0)
n <- 30
...
sv_temp <- 
  data.frame(
    sv = c(
      rgamma(n, shape = 0.7, scale = 100) |> round(),
      rgamma(n, shape = 1.3, scale = 100) |> round()
    ),
    sex =rep(
      c("male", "female"),
      c(n, n)
    ),
    status = c(0, 1) |> sample(n * 2, replace = TRUE)
  )
```


```{r, filename="初期の生存時間に差がある場合"}
survfit(Surv(sv, status) ~ sex, data = sv_temp) |> 
  ggsurvfit() +
  add_confidence_interval()+
  add_censor_mark()
...
Cox回帰では、[30章](chapter30.qmd#aicによるモデル選択)で説明した**AICによるモデル選択**で、説明変数を選択することができます。Rでモデル選択を行う場合には、まず`coxph`関数の`formula`に説明変数をすべて足した式（フルモデル）を設定します。この式を`step`関数の引数に取ることで、AICによるモデル選択の計算が行われます。
...
ハザード比は一般的に**フォレストプロット**と呼ばれる、代表値を点、信頼区間を線で表した図で表記されます。フォレストプロットについては[ggplot2の章](chapter26.qmd#エラーバーの描画geom_errorbargeom_linerangegeom_pointrange)で簡単に紹介しています。
...
フォレストプロットは点と線のグラフですので、[26章](chapter26.qmd#エラーバーの描画geom_errorbargeom_linerangegeom_pointrange)で説明した`geom_pointrange`を使えば簡単に作成できます。

以下のグラフでは、`sex`と`ph.ecog`のハザード比の信頼区間が1に被っていない形になっています。この図から、`sex`、`ph.ecog`のハザード比は1ではなく、いずれも生存時間に統計的に有意な影響を与えていることがわかります。

```{r, filename="フォレストプロットの作図"}
...
<!--
### survivalAnalysisはCRANからremoveされたので、書き直し ###
...
```{r, filename="ハザード比", warning=FALSE, message=FALSE, eval=FALSE}
...
### survminerパッケージ：ggforest関数でフォレストプロットを作成する
...
```{r, filename="ggforest関数でフォレストプロットを作成する"}
pacman::p_load("survminer")
...
```{r, filename="ggsurvlplot_facet関数"}
