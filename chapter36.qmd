...
R markdownとQuartoではファイルの拡張子が異なりますが（R markdownは.rmd、Quartoは.qmd）、どちらも`knitr`でHTMLやPDFに変換される仕組みは同じで、Rではほぼ同じものと考えてよいでしょう。ただし、R markdownではPythonやJulia、Observableを利用できず、Quartoでは利用可能です。PythonやJuliaでは`knitr`ではなく、Jupiterの仕組みを利用して変換されます。

`knitr`ではPythonのコードを実行することができます。このPythonのコードの実行には`reticulate`[@reticulate_bib]というRのライブラリが用いられます。詳しくは[chunkの章](chapter38.qmd)で説明します。
:::
...
このテキストでは、QuartoでHTMLを作成する方法について主に説明します。
...
```{terminal, filename="quarto renderコマンドからRenderする"}
quarto render ファイル名
```
:::
...
小規模のRファイル、解析データなどではProjectは必ずしも必要ではありませんが、この章で説明するQuartoでのHTMLファイル作成や、[47章](chapter47.qmd)で説明するShinyアプリを作成する際、また大規模なデータをいろいろな角度から解析するような場合には、たくさんのコード、画像、表、データソースなどのファイルを用います。このようなたくさんのファイルを管理し、利用する際にはProjectが有用です。Projectをあらかじめ作成しておくことで、後々ファイルの取り扱いが簡単になります。
...
特にQuartoやShinyを利用しない場合、つまり解析などを行うためにProjectを作成する場合には「New Project」を選択しましょう。また、「New Project」を選択した場合でも、後からQuartoやShinyアプリケーションのファイルをProjectのフォルダ内に作成することはできます。
...
R markdownのファイルを作成した場合、R studioはR markdownの例を記載したファイルを作成しますが、qmdファイルではyamlだけのシンプルなファイルが生成されます。
