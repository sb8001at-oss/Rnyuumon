...
Quartoでは、データフレームの表示方法として、`knitr::kable`や`DT::datatable`[@DT_bib]などの、表をきれいに表示するための関数を用いることができます。Rの演算で得られたデータフレームをQuartoで表として表示したい場合には、このどちらかの関数を用いるとよいでしょう。
...
次に、YAMLで引用文献が記載されたbibtexを含むファイル（`references.bib`）を指定する必要があります。[35章](chapter35.qmd#yaml)で説明した通り、YAMLはR markdownやQuartoでファイルの設定を行うためのテキスト形式であり、通常ファイルの一番初めの、`---`で挟まれた部分や`_quarto.yml`に記載します。
...
[bibtex](https://ja.wikipedia.org/wiki/BibTeX)はただのテキストですので、自分で作成することができます。しかし、論文や教科書についての情報を入力したbibtexを自分で作成するのは面倒です。また、ライブラリの引用文をbibtexで準備するのも大変です。
...
```{markdown, filename="文字のサイズを大きくする"}
...
[$$P(y) /cdot P(x|y)$$]{style="font-size: 50%;"}
...
[$$P(y) /cdot P(x|y)$$]{style="font-size: 200%;"}
...
レイアウトの設定では、セミコロン3つ（`:::`）の後に`{}`の中にレイアウトを指定し、レイアウト内の文や図を記載した後、セミコロン3つ（`:::`）で閉じます。このセミコロン3つの間に記載されている部分が、指定したレイアウトで表示されます。
