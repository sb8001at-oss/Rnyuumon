...
Rでは、ライブラリを読み込むことなくグラフを作成することができますが、現在では、[`ggplot2`](https://ggplot2.tidyverse.org/index.html)をグラフ作成に用いるのが事実上のデフォルトのようになっています。`ggplot2`については[後の章](chapter26.qmd)で詳しく説明します。
...
`plot`関数の`x`に因子、`y`に数値を取った場合、`plot`関数は**箱ひげ図**を表示します。箱ひげ図の図の意味は右図の通りです。
...
`rgl`パッケージは、[OpenGL](https://www.opengl.org/) [@woo1999opengl]という、C言語の3Dグラフィックスライブラリを利用したRのグラフ作成ライブラリです。`rgl`パッケージの関数群を用いることで、R上でマウスを使って視点をグリグリ動かすことができる3次元グラフを作成することができます。
...
library(gplots)
venn(list(A = 1:5, B = 3:8))
title(main="Venn Diagram（ベン図）の例")
```

ベン図を描画する場合には、`gplots`ライブラリ[@gplots_bib]の`venn`関数を用います。`venn`関数はリストを引数に取り、リストの各要素を集合としてベン図を描画します。

```{r, filename="gplots::venn関数でベン図を描く"}
...
```{r, error=TRUE, filename="venn関数：要素が5つの場合"}
...
また、`DescTools`ライブラリ[@DescTools_bib]の`PlotVenn`関数でもベン図を描画することができます。`PlotVenn`関数も`venn`関数と同様にリストを引数に取ります。リストの要素の最大値が5つなのも`venn`関数と同じです。また、`col`引数に色を指定することで集合の色を変更することもできます。`col`引数には単に色をベクターで指定することもできますが、`SetAlpha`関数を用いて透明度（`alpha`）を指定することもできます。

```{r, filename="DescTools::PlotVenn関数"}
...
![Rstudioでのグラフの保存](./image/save_plot_Rstudio.png)
