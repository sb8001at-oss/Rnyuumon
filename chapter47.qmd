...
データを解析した後には、その結果を用いて他の人にデータや解析の意味を説明する必要があります。説明するために用いるものの一つが文書やプレゼンテーションです。文書やプレゼンテーションはR markdown・Quarto（[35章～43章](chapter35.qmd)参照）で作成することができます。
...
また、以下のリンクからshinyliveを利用したShinyアプリを利用することもできます（起動に15秒～2分ぐらいかかります）。

- [生存時間解析シミュレーター](https://surv-sim-shinylive.pages.dev/)
- [k-meansのアニメーション](https://kmeans-shinylive.pages.dev/)
- [時系列：ARIMAシミュレーター](https://arima-simulator-shinylive.pages.dev/)
...
通常のWebアプリケーションフレームワークでは、Rの部分に別の言語（RubyやPythonなど）が入り、さらにデータベースと接続されているのが一般的です。Shinyではデータベースは必須ではありませんが、もちろんデータベースを用いたWebアプリケーションを作成することもできます。データベースを用いたアプリケーションを作成する場合には、データベースを取り使うためのパッケージ（[`DBI`]("https://dbi.r-dbi.org/")[@DBI_bib]や[`dbplyr`]("https://dbplyr.tidyverse.org/")[@dbplyr_bib]など）を用いることになります。
...
このような変数・関数の設定やパッケージの読み込みを分離して記載するために用いられるのが**global.R**というファイルです。これは通常のRファイルであり、ui.Rやserver.Rを作成する時のようにRStudioの操作で作成するものではありません。作成中のアプリのフォルダ、ui.Rやserver.Rが保管されている場所にglobal.Rを作成し、変数・関数の設定、パッケージの読み込み等を書き込みます。このglobal.RはShinyアプリを起動する際に実行されるため、ui.Rやserver.Rに余分なスクリプトを書き込む必要がなくなります。同様に、`/R`というフォルダをui.Rやserver.Rと同じフォルダ内に作成し、`/R`内にRのスクリプトを保存することでもアプリ実行時にRのスクリプトを実行することができます。
...
- [shinythemes](https://rstudio.github.io/shinythemes/)[@shinythemes_bib]：ShinyのデザインをBootstrapに従って変更するためのパッケージです。

- [shinydashboard](https://rstudio.github.io/shinydashboard/)[@shinydashboard_bib]：Shinyで[ダッシュボード](https://www.google.com/search?sca_esv=a526368c5945871d&sxsrf=ADLYWIK4gJvNmzBBQh-pQzFeWnI_SKxx2A:1724140556539&q=%E3%83%87%E3%82%B8%E3%82%BF%E3%83%AB%E3%83%80%E3%83%83%E3%82%B7%E3%83%A5%E3%83%9C%E3%83%BC%E3%83%89&udm=2&fbs=AEQNm0BqbPbAzSj6PhNr7nv9Ltx-oFh8tVsgXi1MyFbswNtTUOS5b68chsyOj2QEdx4EPnNHj-rfVa2Eb1VCscGX2ZUj9zg7pD9WhuxXJAj12GW6oeBqw4eGvVgzQxyKznl1ExGJi168qSx2eeXjUNvCmZhFwSaQtcsiZCK7bt8rXXw1KNDgJdghBt7mcicHH5U9gHGYPNgxAD0CsYaK3uh78b4Yn_p-W815iG5byNmUmiBRo1Utr2Q&sa=X&ved=2ahUKEwjP16iLjIOIAxXSslYBHSrNMxYQtKgLegQIGRAB&biw=1912&bih=970&dpr=1)を簡単に作成するためのパッケージです。基本的にはShinyにダッシュボード用のレイアウト関数を追加するパッケージとなっています。

- [bslib](https://rstudio.github.io/bslib/)[@bslib_bib]：`shinythemes`と同様にShinyのデザインを変更するためのパッケージです。こちらは[前章](./chapter38.html)で紹介したR markdown・QuartoのHTML出力のデザインを変更することもできます。また、レイアウトの関数をいくつか備えており、これらのレイアウト関数を用いることでダッシュボードを作成することもできます。`shinythemes`と`shinydashboard`はかなり前に開発されたパッケージであるため、この`bslib`に統合されたような形となっているようです。デザイン性の高いShinyアプリを作成するのであれば、まずこのパッケージを用いることを検討するのがよいでしょう。

- [DT](https://rstudio.github.io/DT/)[@DT_bib]：ShinyやR markdownにJavascript製の表を表示するためのパッケージです。Javascriptで開発され、用いられている[DataTables](https://datatables.net/)というライブラリをRに持ち込んだものです。Shinyで用いるための専用の`Output`、`render`関数を備えています。

- [shinycssloaders](https://daattali.com/shiny/shinycssloaders-demo/)[@shinycssloaders_bib]：Shinyにデータを演算中（ローディング中）であることを示すアニメーションを追加するためのパッケージです。計算が重めのShinyを作成する際に用いるとよいでしょう。

- [shinymanager](https://datastorm-open.github.io/shinymanager/)[@shinymanager_bib]：ShinyにIDとパスワードによるログイン機構を追加するためのパッケージです。セキュリティ的には怪しいので、あまり信用はできませんが、社内のシステムで用いるぐらいなら耐えられるかもしれません。
...
Shinyの開発元であるPositはShinyアプリを簡単にデプロイするサービスである、[Posit Connect Cloud](https://connect.posit.cloud/) や[Posit Connect](https://posit.co/products/enterprise/connect/)を展開しています。また、同じくPositが開発した[Shiny server](https://posit.co/products/open-source/shinyserver/)を用いることで、AWSやGoogle Cloudに比較的簡単にデプロイすることができます。

Posit Connectはエンタープライズ版で、セキュリティ面などがしっかりしています。一方で価格が相当高いので（今は公開されていませんが、昔は$5000/monthぐらいからだったと思います）、個人で安価にデプロイしたいのであればPosit Connect CloudかShiny serverの2択となります。


:::{.callout-tip collapse="true"}
## shinyapps.io

2026年12月31日まではPositは[shinyapps.io](https://www.shinyapps.io/)というサービスを展開していましたが、2027年1月1日以降はConnect Cloudに統合される形でshinyapps.ioのサービスは終了します。

Connect Cloudでは、shinyapps.ioからConnect Cloudへのアプリの移行をサポートする機能が含まれています。Connect Cloudにアカウントを作成し、ログイン後、Admin→shinyapps.ioへと移動します。このページの右上に「Link shinyapps.io account」というボタンがあるので、ココからアカウントをリンクできます。

アカウントが接続できたら、この下の部分にshinyapps.ioで公開していたアプリが表示されるので、右の「migrate」のボタンを押すと、自動的にshinyapps.ioのアプリがConnect Cloudにコピーされます。

![shinyapps.ioからConnect Cloudへアプリを移行する](./image/Connect_cloud_link_to_shinyapps_io.png)

以下は、shinyapps.ioが現役だった時の記事です。残しておきますが、2027年には利用できなくなりますので用いない方がよいでしょう。

### 過去の記事：shinyapps.ioにアプリをデプロイする
...
publishボタンを押すと、初回には2つのパッケージ（`packrat`[@packrat_bib]と`rsconnect`[@rsconnect_bib]）のインストールが求められますので、RStudioの指示に従いパッケージをインストールします。
...
Posit Connect CloudはShinyと[36章で紹介しているQuarto](chapter36.qmd)をデプロイし、公開することのできるサービスです。ShinyやQuartoはPythonにも対応しているので、Pythonで作成したShinyアプリやQuartoの文書もデプロイすることができます。

まずは無料のアカウントをトップページの「Sign Up」から作成しましょう。ただし、無料のアカウントでは月に20時間までしかサーバーでの演算を行うことができません。お金を支払うとサーバーのCPU・メモリを増強／利用時間の延長／組織内のみのアクセス等を追加できるので、必要ならお金を支払って利用するとよいでしょう。

アカウントはGoogleやGithubのアカウントと接続する形でも作成できます。shinyapps.ioのアカウントを同じGoogle/Githubのアカウントで作成している場合、自動的にshinyapps.ioとConnect Cloudのアカウントが接続されます。

![Connect Cloudのトップページ](./image/connect_cloud_toppage.png)

次に、RStudio上でConnect Cloudのアカウントを登録します。OptionのPublishingの項目から、「Connect...」を選択しましょう。登録する際には[`rsconnect`](https://rstudio.github.io/rsconnect/)パッケージ[@rsconnect_bib]が必要です。登録の際に`rsconnect`をインストールするかどうかを尋ねるウインドウが表示されますので、インストールしましょう。

インストール後に再び「Connect...」を選択すると、接続するサービス（Connect Cloud、Posit Connectなど）が表示されるので、Connect Cloudを選択しましょう。選択するとブラウザ上でConnect Cloudへのログインが求められます。ログインするとRStudioの認証パスキーと接続を続けるためのボタンが表示されます。ボタンを押すとアクセスが認証されます（2026年9月時点では認証パスキーの入力を求められないので、ちょっと意味がよくわかりません）。

![RStudioにConnect Cloudのアカウントを登録する](./image/setting_publishing_accounts.png)

後はRStudioのウインドウの右上にある「Publish」ボタンを押すと、Connect Cloudへアプリがアップロードされます。

![Publishボタン](./image/publish_button.png)

この他にGithubにアップロードしたアプリケーションをConnect Cloudからデプロイすることもできますが、Githubを用いるのであれば[Tips](Tips_shinylive.qmd)で紹介している、利用制限の緩いGithub Pagesとshinyliveの組み合わせを用いる方がよいでしょう。

### shiny server

上記の無料のConnect Cloudを用いる方法は非常に簡単で、お金もかかりませんが、一方で演算速度は遅く、たくさんの人が同時に接続することもできません。また、月あたりの使用時間が制限されています。無料のConnect Cloudは基本的にすべての人にShinyアプリを公開する形でデプロイするため、秘密情報を含むShinyアプリに利用するのにも向いていません。お金を払えば制限を撤廃することもできますが、利益を得られないアプリに支払いを続けることも難しい場合があるでしょう。
...
Shinyアプリをブラウザで動かすためのツールが[shinylive](https://posit-dev.github.io/r-shinylive/)です。shinyliveについては[別のTips](Tips_shinylive.qmd)で説明しているので、こちらをご参照ください。shinyliveを用いることで、Shinyアプリを静的サーバー（Github Pagesなどのサービス）にデプロイし、無料で、サーバーの演算能力を利用することなく、Shinyアプリを他の人に利用してもらうことができます。
...
[shinyliveでのサンプルアプリ](https://shiny-sample-shinylive.pages.dev/)
