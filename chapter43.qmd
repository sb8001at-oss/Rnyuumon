...
無料のサービスには、Positが運営しているサービスである[**Connect Cloud**](https://connect.posit.cloud/)や、もっと汎用的にHTMLの公開に用いることができる[**Github Pages**](https://docs.github.com/ja/pages/getting-started-with-github-pages/creating-a-github-pages-site)などがあります。
...
とは言え、個人で作成するようなWebsiteやBookではアクセス制限に到達したり、HTMLのサイズが大きくなりすぎることはあまりないと思います。Quartoで作成したHTMLを公開する場合、とりあえずConnect CloudかGithub Pagesでデプロイすることを検討するとよいでしょう。

## Connect Cloud

[Connect Cloud](https://connect.posit.cloud/)にQuartoのHTMLをデプロイする場合には、まずConnect Cloudのアカウントを作成する必要があります。右上の「Sign Up」からアカウントを作成しましょう。Sign UpにはGooGleやGithubのアカウントを利用することもできます。

![](./image/connect_cloud_toppage.png)
...
後はRStudioのウインドウの右上にある「Publish」ボタンを押すと、Connect CloudへHTMLがアップロードされます。

![Publishボタン](./image/publish_button.png)

この他にGithubにアップロードしたqmdファイルやHTMLファイルをConnect Cloudからデプロイすることもできますが、Githubを用いるのであれば以下に紹介するより利用制限の緩いGithub Pagesを用いる方がよいでしょう。

:::{.callout-tip collapse = "true"}
## Quarto Pub

Quarto Pubは2026年12月31日までは存続し、2027年1月1日からはConnect Cloudへ移行する予定です。[47章](chapter47.qmd)で説明するshiny.ioと共に、元々は別々のサービスとして運営されていたものをConnect Cloudに一元化することを目的としているようです。shiny.ioのアプリは比較的簡単にConnect Cloudに移行できるようにされていますが、Quarto Pubからの移行の方法は現状提供されていません。ですので、Quarto Pubにデプロイ済みの文書をConnect Cloudにデプロイする場合には、元々のファイルをRStudioで開き、上記の「Publish」の機能を用いてアップロードし直す必要があります。

以下にQuarto Pubの使い方についての記事を残しておきますが、2026年12月末以降は新しくアカウントを作ることができませんので、Connect Cloudを利用するとよいでしょう。

### Quarto Pubに文書をデプロイする（～2026年12月31日） {.unnumbered}

[Quarto Pub](https://quartopub.com/)はPositが運営しているサービスで、Rstudioを用いていれば簡単にHTMLをデプロイすることができます。
...
```{terminal, filename="quarto publishコマンド"}
...
### アクセストークン（Access Token） {.unnumbered}
...
### Git：ユーザー名とメールアドレスの登録 {.unnumbered}
...
### GithubのリモートリポジトリとGitを関連付ける {.unnumbered}
