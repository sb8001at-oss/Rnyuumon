...
`stars`は`sf`の開発者が作成したラスタデータの取り扱いに関するライブラリです。`terra`と比べるとやや[ダウンロードされていないパッケージ](https://www.datasciencemeta.com/rpackages)ではありますが、ココでは`terra`ではなく`stars`について説明することとします。
...
インタラクティブな地図の表示には、後ほど説明する`leaflet`パッケージでも利用されている、Javascriptの地図表示ライブラリである[leaflet](https://leafletjs.com/)が使用されています。
