...
また、統計手法としてはt検定や一元分散分析、単回帰などを用いることができます。
...
説明変数が2つや3つの場合であれば、2元や3元の分散分析で解析することができます。交互作用もそれほど多くありませんし、総当たりの実験を組んだとしても、それほど実験の数は増えないでしょう。
...
作物の例でスクリーニングを考えてみましょう。目的変数は作物の収量（重さ）とします。説明変数には、肥料の量、日照量、水の量、土壌のpH、土壌の粒の細かさを選ぶこととします。目的変数は1つで、説明変数は5つです。
...
目的変数の行列を$Y$、説明変数の行列を$X$、パラメータの行列を$\beta$、ばらつきの項を$u$と置き換えると、以下の式で表すことができます。

$$ Y = X \beta + u $$

$\beta$の真の値はわからないため、$\beta$の予測値（$\hat{\beta}$、ベータハット）を$\beta$の変わりに用いることにします。また、$X$は実験のデザインを表す行列として、**デザイン行列**と呼びます。
...
ただし、x1の2乗が含まれているなら線は2次関数のような曲線になりますが、x1は-1と1の2つの値しか取っていないため、このデータではyとx1の関係について、どのように曲がっているのかはわかりません。つまり、2点では多項式のパラメータは求まりそうにありません。
...
```{r}
d[c(1, 4), ] # 2、3行目を削除する
lm(weight ~ nutrient * water, data = d[c(1, 4), ])
```
...
ggplot(d[c(1, 4), ], aes(x = nutrient, y = weight, color = factor(water))) + geom_point(size=3) + geom_line() + labs(title="2実験を行う場合2")
```

### パラメータの標準誤差の計算

ただし、4つのデータで4つのパラメータ（説明変数2つの傾き、交互作用1つ、切片1つ）を求めると、標準誤差が求まらなくなります。これは、下の演算の結果に示されている通り、残差の自由度が0になるからです。

```{r, filename="要員配置実験での標準誤差"}
lm(weight ~ nutrient * water, data = d) |> summary() # Std.Errorが求まらない（NaN）
```

要因配置計画でも、主効果のみを計算すると、交互作用に割り振る自由度がなくなるため、標準誤差が求まります。
...
FrF2(nfactor = 6, estimable=formula("~(A+B+C)^2+D+E"), clear = FALSE) # clearがないと演算できない
...
`FrF2`関数では、`ncenter`引数を用いることで、中間点を加えることができます。`ncenter`には数値を設定し、設定した数だけ中間点のみの実験を追加します。また、すでに作成した`FrF2`関数の返り値を引数に取る、`add.center`関数でも中間点を設定することができます。
...
`DanielPlot`関数は、論文[@Daniel01111959]で説明されている、正規分布と実際の効果を大きさ順にならべ、プロットしたものです。説明変数に何の効果も無ければ、直線上に点が並びます。正規分布のマイナス側の絶対値を取ったプロットにすることもでき（`half = TRUE`）、こちらの方が直線に点が乗っているかわかりやすくなります。説明変数に効果がある場合には、直線より右側に点が外れることになります。

上記に回帰のコードを示した通り、目的変数yはただの正規乱数ですので、どの説明変数にも明確な効果がなく、すべての点が直線上に乗ります。

```{r, filename="Daniel Plot"}
# 直線に乗らない説明変数には有意な効果があると考える
DanielPlot(lm_xy)
DanielPlot(lm_xy, half = TRUE) # 
```
...
説明変数がn個のとき、要因配置計画では2^n^、一部要因配置計画では2^n-k^（k<n）の実験を行うことになります。とはいえ、実験で必ず2の倍数で実験を行うかというと、そういうわけではないでしょう。時間や予算によっては2^n^の実験を実施することは難しくても、2^n-1^よりは多く実験ができそう、という場合もあります。実験を増やせば結果の精度が上がっていきますから、わざわざ一部要因配置計画に合わせて実験をする必要はないでしょう。また、`FrF2`では各説明変数に2条件を仮定していましたが、3条件以上で実験を行い、説明変数への応答が直線的なのか、曲線的なのか評価したい場合もあるでしょう。
...
- 各説明変数の和は決まっていて、説明変数の割合を変える場合（例えばホットケーキミックス500g中の小麦粉やベーキングパウダーなどの原材料の量）を想定したもの
- 実験を追加で行うことを想定したもの
...
このグラフのFraction of Design Spaceが0.5の時のPrediction Variance（赤線が交わる点）をできるだけ小さくするモデルを選択するのがI-Optimal Designです。I-Optimal Designは説明変数と目的変数の関係が多項式的（直線ではなく、曲線的）である場合に比較的適しているようです。

D-Optimal Design、I-Optimal Design共に最適なデザイン行列は乱数を使った繰り返し計算（Monte Carloアルゴリズム）で計算できます。一部要因配置計画についてはコンピュータが発達する前から直交表を用いて作成されてきましたが、Opitimal Designではデザイン行列の作成に繰り返し計算を用いるため、デザインの作成にはコンピュータが必要です。
...
実験を計画する場合、説明変数はいつも自由に変更できる、というわけではありません。例えば、高温に溶かした素材を用いて製品を作成する場合、窯の温度をランダムに上げたり下げたりすると燃料のコストも時間もかかってしまいます。したがって、説明変数の一部はあまり頻繁に変更せず、実験を行った方がよい場合もあります。
...
get_optimality(od_d) # D-OptimalではDが大きい
get_optimality(od_i) # I-optimalではIが小さい
```
...
説明変数全体の量は決まっていて、説明変数の割合を変える場合（例えばホットケーキミックス500g中の小麦粉やベーキングパウダーなどの原材料の量）のことを**mixture experiment**と呼びます。mixture experimentでは、各説明変数の和が常に1となるよう実験計画が作成されます。
