...
```{r, echo=FALSE}
d <- data.frame(
  isfunc = c("is.numeric(x)", "is.integer(x)", "is.double(x)", "is.complex(x)", "is.character(x)", "is.logical(x)", "is.factor(x)", "is.atomic(x)", "is.list(x)", "is.matrix(x)", "is.data.frame(x)", "is.na(x)", "is.nan(x)", "is.null(x)", "is.infinite(x)"),
  meaning = c("数値型", "整数型", "double型", "複素数型", "文字列型", "真偽型", "因子型", "ベクター", "リスト", "マトリックス", "データフレーム", "NA", "NaN", "NULL", "無限大")
)

colnames(d) <- c("is.関数名", "チェックする型")
knitr::kable(d, caption="表1：xの型の確認に用いる関数")
```
...
```{r, echo=FALSE}
d <- data.frame(
  asfunc = c("as.numeric(x)", "as.integer(x)", "as.double(x)", "as.complex(x)", "as.character(x)", "as.logical(x)", "as.factor(x)", "as.Date(x)", "as.POSIXct(x)", "as.POSIXlt(x)", "as.list(x)", "as.matrix(x)", "as.data.frame(x)", "as.null(x)"),
...
knitr::kable(d, caption="表2：xの型の変換に用いる関数")
```
