...
- [日本の人口](https://01a075bc-d23a-c6c6-f744-06b6fff6bb70.share.connect.posit.cloud/)：これはGallaryに示されているものではなく、私が作成したものです。[ココ](https://github.com/sb8001at-oss/Rnyuumon/blob/master/sample_observable/index.qmd)にqmdファイルを保存しています。Observableを利用して作成しており、スマホで表示するとグラフが際限なく縦長になります。
...
Shinyについては[47章](chapter47.qmd)で説明しています。また、`plotly`に関しては[27章](chapter27.qmd)、`leaflet`に関しては[45章](chapter45.qmd#leafletパッケージ)で説明しています。この章では主にダッシュボードの構造とObservableについて説明します。
...
また、以下のように`{.card}`を用いてカードを設定することもできます。この`{.card}`を用いる場合、`title`をブランケット（`{}`）の中に書くことで指定できます。
