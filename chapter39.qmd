...
ただし、この`code-link`には`downlit`[@downlit_bib]と`xml2`[@xml2_bib]の2つのパッケージが必要な上、base Rの関数ぐらいにしかリンクがつきません。また、`knitr`以外のレンダリングエンジン（`jupyter`など）を用いる場合、つまりR以外の言語を用いる場合には利用できません。おまけぐらいに考えておくとよいでしょう。
