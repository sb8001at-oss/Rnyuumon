...
以下では、S4について書かれたテキスト[@Genolini_2008]と[AppsilonのS4についてのポスト](https://www.appsilon.com/post/object-oriented-programming-in-r-part-3)、[Advanced RのS4の章](https://adv-r.hadley.nz/s4.html)[@Wickham_Hadley2019-05-24]を参考にS4について説明していきます。

S4もS3と同じくオブジェクト指向プログラミングのためのクラスです。S3でのクラス定義の基本的な概念はS4にも通じます。S4でも、S3と同じく以下の要素をクラスの定義として記載していくことになります。
...
library(readxl)
d <- read_excel("./data/S4_BasicClass.xlsx")
knitr::kable(d, caption = "Slotsに指定可能なデータ型")
```
...
```{r, filename="new関数：Slotsを引数で指定する"}
...
```{r, error=TRUE, filename="Slotsに値を設定する"}
Pochi@name <- "Pochi"
...
```{r, filename="prototypeでSlotsのデフォルト値を設定する"}
setClass(
  Class = "MyDog", 
  slots = c(
    name = "character",
...
```{r, filename="クラスの削除：removeClass"}
removeClass("MyDog")
```

## バリデータ

上記の通り、S4ではSlotsのデータ型を指定できます。その他のSlotsの値の制限、例えばSlotsに設定可能な値の長さや範囲などは、別途`validity`引数に指定します。この`validity`引数は[S3で説明したバリデータ](Tips_S3class.qmd#バリデータvalidator)に当たるものです。

`validity`引数は関数で指定します。この関数で指定した返り値が`FALSE`になった時には、オブジェクト作成時にエラーが出ます。

```{r, filename="バリデータの設定：validity引数"}
setClass(
  Class = "MyDog", 
  slots = c(
    name = "character",
...
```{r, filename="バリデータでFALSEが出る場合", error=TRUE}
...
```{r, filename="validityに条件を2つ以上指定する"}
setClass(
  Class = "MyDog", 
  slots = c(
    name = "character",
...
```{r, filename="validObject関数でオブジェクトの状態を確認する"}
...
```{r, filename="setValidity関数でバリデータを設定する"}
setValidity(
  Class = "MyDog", 
...
```{r, filename=="setValidityはクラスのバリデータを書き換える", error=TRUE}
...
```{r, error=TRUE, filename="クラスを定義し直すと、定義前のクラスオブジェクトがエラーとなる"}
setClass(
  Class = "MyDog", 
  slots = c(
    namae = "character",
...
まずは`setMethod`関数でオブジェクト作成時に実行される関数（**initializer**）を作成します。initializerはその関数の名前である`initialize`で呼び出すのではなく、`new`関数が呼び出されたときに実行される関数です。クラスオブジェクトを作成したときに表示される文字列やSlotsに設定するattributes（ベクターの名前や行列・データフレームのdim、列名など）の設定を定義しておくと良いでしょう。
...
```{r, filename="オブジェクト作成時に実行される関数：initialize"}
...
S4ではクラスオブジェクトを`new`関数で作成できますが、この`new`関数はクラス名と結びついておらず、クラスオブジェクトを作成する場合には扱いにくい関数です。

[S3の章](Tips_S3class.qmd#ヘルパーhelper)で説明した通り、Rでは関数名と作成するオブジェクトのクラスが一致する場合が多いです。例えば、`factor`を作成するときには`factor`関数、`data.frame`を作成するときには`data.frame`関数を用いています。
...
```{r, filename="setClass関数でコンストラクタを設定する"}
...
```{r, filename="既存の関数にジェネリック関数を設定する"}
setMethod(
  f = "show",
...
```{r, filename="S4のジェネリック関数を表示：showMethods"}
showMethods("show")
```
...
```{r, error=TRUE, filename="新しい名前のジェネリック関数をsetMethodで作成"}
setMethod(
  f = "ageDays",
  signature = (x = "MyDog"),
...
この`setGeneric`関数は関数名と関数の内容を引数に取りますが、関数の内容としては必ず`standardGeneric`関数を呼び出す関数を指定することになっています。[S3における`UseMethod`](Tips_S3class.qmd#新規のジェネリック関数を作成する)のようなものだと思っておくとよいかと思います。

この`setGeneric`関数で宣言した関数名はRの`methods`関数で探索できるようになります．

```{r, filename="ジェネリック関数名を定める：setGeneric関数"}
...
```{r, eval=FALSE, filename="lockBinding関数でsetGenericを再宣言できなくする"}
...
```{r, filename="setGeneric関数で関数名を指定する"}
...
```{r, filename="setMethod関数でゲッターを定義する"}
setMethod(
  f = "getName",
...
```{r, filename="`[`にジェネリック関数の設定を加える：引数が文字列"}
setMethod(
  f = "[",
...
```{r, error=TRUE, filename="[]でスロットを取り出す（文字列）"}
Pochi["name"]
Pochi["age"]
Pochi["birthday"]
...
```{r, filename="`[`にジェネリック関数の設定を加える：引数が数値"}
setMethod(
  f = "[",
...
```{r, filename="数値・文字列の両方のインデックスで値を取り出す"}
Pochi[1]
Pochi[2:3]
...
```{r, filename="setReplaceMethodで関数を定義する"}
setReplaceMethod(
  f = "setName",
...
```{r, error=TRUE, filename="new関数でオブジェクトを作成できない"}
...
```{r, filename="サブクラス用のinitializerを作成する"}
setMethod(
  f = "initialize",
  signature = "BdMyDog",
...
```{r, filename="スーパークラス：PoliceDogLicense", message=FALSE}
...
```{r, filename="多重継承：BdMyDogとPoliceDogLicenseを継承する"}
setClass(
  "PoliceDog",
...
```{r, filename="多重継承：initializerを定義する"}
#| code-fold: TRUE
...
多重継承したクラス（`PoliceDog`）のオブジェクトをスーパークラスに含まれるSlotsを設定して作成することができます。

```{r, filename="多重継承：オブジェクトを作成する"}
temp <- 
  new(
    "PoliceDog", 
    name = "Pochi", 
...
```{r, filename="PoliceDogLicenseクラスの構造"}
str(temp)
```

また、スーパークラスで設定したジェネリック関数である`getName`を適用することができます。

```{r, filename="スーパークラスで設定したジェネリック関数を利用する"}
getName(temp)
```
...
[46章](chapter46.qmd#adjacency-matrixからグラフを作成する)で説明した`igraph`[@igraph_bib1]とクラスの継承をadjacency matrixで評価してくれる`classesToAM`を用いると、クラスの継承をグラフで示すことができます。

```{r, message=FALSE, filename="classesToAM関数で継承のグラフを作成する"}
library(igraph)
...
ですので、多重継承する場合にはクラスの設計、クラスの関数名、引数に使うクラスの性質、実際に適用される関数に気を配る必要があります。
...
```{r, error=TRUE, filename="as関数で全く異なるクラスに変換しようとする"}
as(Pochi, "character")
```

このような場合、データの変換の方法をあらかじめ`setAs`関数で指定しておくと変換することができます。

```{r, filename="変換のルールを定める：setAs関数"}
...
```{r, error=TRUE, filename="initializerがあるためエラーが出るケース"}
as(Pochi, "BdMyDog")
```
...
```{r, error=TRUE, "filename=setIs関数でスーパークラスを設定する"}
...
# スーパークラスのSlotsをすべて備えている時はエラーが出ない（すでにisの方法があるとwarningがでる）
setIs("ClassB", "ClassA")
...
```{r, filename="ANYを指定するとどのようなデータ型も登録できてしまう"}
...
```{r, filename="SlotsにClass Unionを指定する", error=TRUE}
...
最後にVIRTUALクラスについて説明します。すでに @sec-basic に示していた通り、basicクラスのいくつかはVIRTUALクラスとして設定されています。

`setClass`関数でSlotsを設定しない時と、`contains = "VIRTUAL"`を指定した場合には、そのクラスはVIRTUALクラスとなります。

```{r, filename="VIRTUALクラスを作成する"}
...
```{r, error=TRUE, filename="VIRTUALクラスではオブジェクトを作成できない"}
new("ClassE")
new("ClassF")
```
...
```{r, filename="AnimalクラスをVIRTUALクラスとして作成する"}
setClass(
  Class = "Animal", 
...
```{r, filename="VIRTUALクラスのオブジェクトは作成できない", error=TRUE}
new("Animal")
```

しかし、このAnimalクラスを継承したクラスでは、Animalクラスと同じSlotsを持つオブジェクトを作成することができます。

```{r, filename="VIRTUALクラスを継承する"}
