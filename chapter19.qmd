...
無名関数とは、関数名を定義することなく用いる関数で、`apply`関数などで利用されます（[7章](./chapter7.html)の最後を参照）。無名関数は`function`でも、バックスラッシュ（`\`）でも作成できます。
...
[16章](./chapter16.html)で紹介したパイプ演算子は演算の文脈を理解するために有用ですが、パイプ演算子の利用が適していない場合もあります。例えば、
...
[22章](./chapter22.html)で紹介する`ggplot2`では、`+`演算子ごとに改行を入れます。また、引数がたくさんある場合には改行を入れるようにしましょう。
