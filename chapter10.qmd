...
最近では、最新のパッケージはCRANだけでなく、[**GitHub**](https://github.co.jp/)というプログラム開発プラットフォームからインストールすることもあります。ただし、**GitHub上のパッケージはCRANによるチェックを受けていない**ものですので、インストールする際には注意が必要です。GitHubからのパッケージのインストールには、`devtools` [@devtools_bib]パッケージの`install_github`関数を用います。引数には、パッケージの**リポジトリ**というものを文字列で取ります。
