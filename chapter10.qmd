...
Rを含めた多くのプログラミング言語では、nativeな言語にはない機能を後から追加することができます。この追加する機能のセットのことを、**ライブラリ**と呼びます（**パッケージ**と呼ぶこともあります）。
...
ライブラリには、読み込み時にメッセージを表示するものがあります。例えば上記の`tidyverse`パッケージは読み込み時にメッセージが表示されます。

ライブラリ読み込み時のメッセージを表示させたくない場合には、`suppressPackageStartupMessages`という関数を用います。

```{r, filename="メッセージを表示せずにライブラリを読み込む", eval=FALSE}
