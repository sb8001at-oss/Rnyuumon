...
d <- data.frame(
  func = c("sum(x)", "length(x)","mean(x)", "var(x)", "sd(x)", "median(x)", "max(x)",
           "min(x)","quantile(x, probs)", "IQR(x)",
           "cov(x, y)", "cov(data.frame)",  "cor(x, y)", "cor(data.frame)"),
  meaning = c("合計値", "ベクターの長さ", "平均値", "（不偏）分散", "（不偏）標準偏差", "中央値", "最大値", "最小値", "分位値（probsは分位の位値）", "25%分位値と75%分位値の差","共分散", "分散・共分散行列", "相関係数", "相関行列")
)
...
```{r, filename="polyroot関数とuniroot関数", error=TRUE}
