...
Rの`round`関数は概ね四捨五入の結果を返しますが、正確には四捨五入にはなっていない場合があるので、注意が必要です。`0.5`を四捨五入すると`1`ですが、`round(0.5)`は`0`を返します。詳しくは`?round`で表示されるヘルプのDetailsをご参照下さい。

```{r, filename="Rのroundの仕様"}
round(0.49)
round(0.5) # 0になる
round(0.51)
```
