...
**教師なし学習**とは、取れたデータをすべて用いて、似ているデータごとに群として分けるような手法です。教師なし学習では、訓練データと予測を行うデータを明確に分けることはありません。教師なし学習による群分けには、**次元圧縮**と**クラスタリング**があります。教師なし学習については[次の章](./chapter32.html)で詳しく解説します。
...
```{r, filename="SVM：e1071パッケージの読み込み"}
pacman::p_load(e1071)
```

### 訓練データとテストデータ

教師あり学習の機械学習を用いる場合には、まずデータを**訓練データ**と**検証データ（テストデータ）**に分けるのが一般的です。これは[前章](./chapter30.qmd#部分的最小二乗回帰)の部分的最小二乗回帰（PLS）の項で説明したものと同じです。
...
[前の章](chapter30.qmd#ニューラルネットワーク回帰)でも紹介した**ニューラルネットワーク**は、分類でも用いられる機械学習の手法です。目的変数にカテゴリカルデータを用いる点が回帰とは異なります。
