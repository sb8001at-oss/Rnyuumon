...
各postにはタイトルや著者、登校日、ブログのカテゴリ、トップ画像を設定することができます。タイトルや著者には、[39章](chapter39.qmd#文章のタイトルと著者)に記載した通り、`title`や`author`、`date`をそのpostのYAMLに設定します。ブログのカテゴリは`categories`に、ブログのトップ画像として用いる画像は`image`にそれぞれ設定します。
