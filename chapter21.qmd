...
**時系列データ（time series data）**は、日時データとセットになった測定値です。例えば、[18章](chapter18.qmd#nile)で説明した`Nile`のデータセットは時系列データの代表的な例です。`Nile`は1871年から1970年のナイル川の流量を年ごとに測定したデータです。
...
Rでの日時データの取り扱いを簡単にするためのパッケージが、[`lubridate`](https://lubridate.tidyverse.org/)パッケージ [@lubridate_bib]です。`lubridate`パッケージは日時データを取り扱うための関数のセットを提供しており、様々なフォーマットの文字列を簡単に日時データに変換し、演算に用いることができます。
